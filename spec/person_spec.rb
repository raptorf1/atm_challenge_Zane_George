...
describe Person do
    ...
    describe "can manage funds if an account has been created" do
        ...
        it "can withdraw funds" do
            command = lambda { subject.withdraw_funds(amount: 100, pin: subject.account.pin_number, account: subject.account, atm: atm) }
            expect(command.call).to be_truthy
        end

        it "withdraw is expected to raise error if no ATM is passed in" do
            command = lambda { subject.withdraw_funds(amount: 100, pin: subject.account.pin_number, account: subject.account)}
        ...
        it 'funds are added to cash - deducted from account balance' do
            subject.cash = 100
            subject.deposit(100)
            subject.withdraw_funds(amount: 100, pin: subject.account.pin_number, account: subject.account, atm: atm)
